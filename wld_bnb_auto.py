...
STATE_FILE = os.path.join(SCRIPT_DIR, 'wld_bn_state.json')
LOG_FILE = os.path.join(SCRIPT_DIR, 'wld_bn.log')
TRADE_LOG = os.path.join(SCRIPT_DIR, 'wld_bn_trades.txt')
...
def log_trade(entry):
    ...
    if action == 'OPEN':
        lines += [
            f'操作: 开仓{d_cn}',
            f'数量: {entry.get("qty")} WLD | 杠杆: {entry.get("leverage")}x',
...
class Executor:
    ...
    def close_position(self, position_side):
        try:
            ...
            log(f'平仓: {position_side} {amt} WLD')
            cr = {
                'action': 'CLOSE',
                'symbol': 'WLD/USDT',
...
def main():
    log('══════ WLD自动交易 启动 (币安 10x) ══════')
    log(f'品种: {SYMBOL}  仓位: {POSITION_SIZE} WLD  轮询: {POLL_SECONDS}s')
                    ...
                    try:
                        ...
                        if os.path.exists(epath):
                            with open(epath) as _f:
                                edata = _json.load(_f)
                                enrich = edata.get('coins', {}).get('WLD', {})
                        ...
                        decision, reason = llm_analyze(
                            'WLD', direction, plan['entry'], plan['sl'],
                        ...
                        _write_trade_log('WLD', 'REJECTED', f'LLM异常: {str(e)[:60]}', {'direction': direction, 'entry_price': plan['entry'], 'stop_loss': plan['sl'], 'take_profit': plan['tp'], 'qty': POSITION_SIZE, 'leverage': LEVERAGE})
