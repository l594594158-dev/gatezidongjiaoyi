            ...
            if direction and not executor.has_position(direction) and plan:
                if executor.has_open_order(direction):
                    ...
                    try:
                        ...
                        _write_trade_log('NEAR', 'REJECTED', f'LLM异常: {str(e)[:60]}', {'direction': direction, 'entry_price': plan['entry'], 'stop_loss': plan['sl'], 'take_profit': plan['tp'], 'qty': POSITION_SIZE, 'leverage': LEVERAGE})
