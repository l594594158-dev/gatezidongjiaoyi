            ...
            if '-4045' in err:
                ...
                try:
                    ...
                    if isinstance(all_algos, list):
                        for aa in all_algos:
                            try:
                                p3 = sign2({'symbol': 'ZECUSDT', 'algoId': aa['algoId']})
        ...
        try:
            ...
            for pos in self.ex.fetch_positions([SYMBOL]):
                ...
                for a in active_algos:
                    ...
                    if not has_match:
                        try:
                            p3 = signed({'symbol': 'ZECUSDT', 'algoId': a['algoId']})
                    ...
                    try:
                        ...
                        if os.path.exists(epath):
                            with open(epath) as _f:
                                edata = _json.load(_f)
                                enrich = edata.get('coins', {}).get('ZEC', {})
                        ...
                        decision, reason = llm_analyze(
                            'ZEC', direction, plan['entry'], plan['sl'],
                            plan['tp'], POSITION_SIZE, LEVERAGE, indicators, enrich
                        )
                
                        if decision == 'CONFIRMED':
                            log(f'LLM确认: {reason[:60]}')
                            _write_trade_log('ZEC', 'CONFIRMED', reason, {'direction': direction, 'entry_price': plan['entry'], 'stop_loss': plan['sl'], 'take_profit': plan['tp'], 'qty': POSITION_SIZE, 'leverage': LEVERAGE})
                            executor.open_position(plan)
                            state['last_signal'] = direction
                            save_state(state)
                        else:
                            log(f'LLM否决: {reason[:60]}')
                            _write_trade_log('ZEC', 'REJECTED', reason, {'direction': direction, 'entry_price': plan['entry'], 'stop_loss': plan['sl'], 'take_profit': plan['tp'], 'qty': POSITION_SIZE, 'leverage': LEVERAGE})
                    except Exception as e:
                        log(f'LLM异常({e})，安全拒绝开仓')
                        _write_trade_log('ZEC', 'REJECTED', f'LLM异常: {str(e)[:60]}', {'direction': direction, 'entry_price': plan['entry'], 'stop_loss': plan['sl'], 'take_profit': plan['tp'], 'qty': POSITION_SIZE, 'leverage': LEVERAGE})
