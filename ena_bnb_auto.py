...
STATE_FILE = os.path.join(SCRIPT_DIR, 'ena_bn_state.json')
LOG_FILE = os.path.join(SCRIPT_DIR, 'ena_bn.log')
TRADE_LOG = os.path.join(SCRIPT_DIR, 'ena_bn_trades.txt')
...
def log_trade(entry):
    ...
    if action == 'OPEN':
        lines += [
            f'操作: 开仓{d_cn}',
            f'数量: {entry.get("qty")} ENA | 杠杆: {entry.get("leverage")}x',
...
class Executor:
    ...
    def close_position(self, position_side):
        try:
            ...
            log(f'平仓: {position_side} {amt} ENA')
            cr = {
                'action': 'CLOSE',
                'symbol': 'ENA/USDT',
            ...
            if '-4045' in err:
                ...
                try:
                    ...
                    if isinstance(all_algos, list):
                        for aa in all_algos:
                            try:
                                p3 = sign2({'symbol': 'ENAUSDT', 'algoId': aa['algoId']})
        ...
        try:
            ...
            for pos in self.ex.fetch_positions([SYMBOL]):
                ...
                for a in active_algos:
                    ...
                    if not has_match:
                        try:
                            p3 = signed({'symbol': 'ENAUSDT', 'algoId': a['algoId']})
...
def main():
    log('══════ ENA自动交易 启动 (币安 10x) ══════')
    log(f'品种: {SYMBOL}  仓位: {POSITION_SIZE} ENA  轮询: {POLL_SECONDS}s')
                    ...
                    try:
                        ...
                        if os.path.exists(epath):
                            with open(epath) as _f:
                                edata = _json.load(_f)
                                enrich = edata.get('coins', {}).get('ENA', {})
                        ...
                        decision, reason = llm_analyze(
                            'ENA', direction, plan['entry'], plan['sl'],
                            plan['tp'], POSITION_SIZE, LEVERAGE, indicators, enrich
                        )
                
                        if decision == 'CONFIRMED':
                            log(f'LLM确认: {reason[:60]}')
                            _write_trade_log('ENA', 'CONFIRMED', reason, {'direction': direction, 'entry_price': plan['entry'], 'stop_loss': plan['sl'], 'take_profit': plan['tp'], 'qty': POSITION_SIZE, 'leverage': LEVERAGE})
